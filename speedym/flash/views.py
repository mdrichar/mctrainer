...
def index(request):
    return HttpResponse("Hello, World. You're at the new flash index.")
...
def subsetq(request):
    ...
    return HttpResponse("Hello, World. subsetq")

def listing(request):
    #factoid_listing = Factoid.objects.all()
    factoid_listing = BinaryFact.objects.filter(qsets__id=3)
